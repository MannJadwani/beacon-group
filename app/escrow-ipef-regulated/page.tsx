...
function parseEscrowIpef(markdown: string): {
  title: string;
  intro: string;
  coverage: string[];
  ...
  const start = lines.findIndex((l) => l.trim() === "# Escrow Services: Investor Protection Fund Escrow");
...
export default function EscrowIpefRegulatedPage() {
  const mdPath = path.join(process.cwd(), "content", "escrow-ipef-regulated", "index.md");
  const md = fs.readFileSync(mdPath, "utf8");

  const { title, intro, coverage, sections, alsoOffer, offices } = parseEscrowIpef(md);
        ...
        <div className="wrapper relative z-10 px-5 pb-10 pt-32 lg:px-20 lg:pb-16 lg:pt-40">
          <div className="grid grid-cols-1 gap-14 lg:grid-cols-12 lg:gap-16">
            <div className="lg:col-span-7" data-aos="fade-right">
              ...
              >
                {intro ||
                  "Beacon Trusteeship acts as an independent escrow agent for investor protection funds, enforcing clear conditions for asset release."}
              </p>
              ...
              <div className="bg-white/10 p-px">
                <div className="bg-primary-navy/70 p-8 lg:p-10">
                  ...
                  <h2 className="mt-5 text-2xl font-medium leading-tight text-white">
                    Safeguarded cash flows for investor protection.
                  </h2>
                  <p className="mt-4 text-sm leading-relaxed text-white/70">
                    We act as a neutral trustee for escrow accounts and security custody across regulated use cases.
                  </p>
                  <ul className="mt-8 grid grid-cols-1 gap-3 text-sm text-white/80">
                    {(coverage.length > 0 ? coverage : ["Debt servicing", "Private equity transactions", "Retention of securities", "Loan against securities"]).map(
                      (item) => (
                  ...
                  <div className="bg-primary-navy/10 p-px">
                    <div className="bg-white p-8">
                      ...
                      <ul className="mt-6 space-y-3 text-sm text-primary-navy/70">
                        {(coverage.length > 0 ? coverage.slice(0, 4) : ["Debt servicing", "Private equity", "Retention of securities", "Loan against securities"]).map(
                          (t) => (
